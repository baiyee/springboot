package com.zx.springboot.utils.core.json;
...
public abstract class JsonUtils{
    ...
    public static String toJSONStringConvertNull(Object object) {
        return toJSONString(object,SerializerFeature.DisableCircularReferenceDetect,SerializerFeature.WriteNullStringAsEmpty,SerializerFeature.WriteMapNullValue, SerializerFeature.WriteNullListAsEmpty);
    }
...
//            MethodUtils.invokeMethod(builder, methodName, pb);
     ...
     * @return 返回对应的java实体
     */
    public static <T extends JsonParse> T jsonToPojo(JSONObject jsonObject, Class<T> klass) {
        ...
        for (Field fd : fds) {
            ...
            try {
                ...
                log.warn("json格式和pojo类型不一致 pojo字段: "+ fd.getName()+ " json: "+ JsonUtils
                        .toJsonString(jsonObject.getString(fd.getName())).replaceAll("\"", "'").replaceAll("\\\\'", "\""));
            }
        }
        return obj;
    }
